...
export const Products: React.FC<ProductsProps> = ({ onEdit, onAddNew, onViewDetails }) => {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      const matchesStatus = statusFilter === '' || product.availability === statusFilter;
  ...
  const tableGridClass = "grid grid-cols-[1fr_200px_150px_150px_90px] items-center px-6";
      ...
      <div className="flex flex-col w-full bg-white flex-none border-b border-neutral-200">
        <div className="flex flex-row items-center p-[12px_20px] lg:p-[12px_24px] gap-[12px] lg:gap-[16px] w-full min-h-[64px] lg:min-h-[72px]">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button variant="primary" className="!h-[36px]" leftIcon="ph ph-plus" onClick={onAddNew}>Novo Produto</Button>
          </div>
        </div>
        ...
        <div className="bg-white border border-neutral-200 shadow-small rounded-[12px] overflow-hidden">
                  ...
                  paginatedProducts.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-4 overflow-hidden">
                        ...
                        <span className="text-body2 font-bold text-neutral-black truncate group-hover:text-primary-700 transition-colors">{p.name}</span>
                      </div>
                      <span className="text-body2 font-bold text-neutral-950 truncate">{p.category}</span>
                      <span className="text-body2 font-bold text-neutral-950 tabular-nums">{p.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
                      <div className="flex justify-center">
                        <Badge variant={p.availability === 'ATIVO' ? 'success' : 'neutral'}>{p.availability}</Badge>
                      </div>
          ...
          <div className="flex items-center justify-between p-4 border-t border-neutral-100 bg-white">
            <span className="text-body2 font-medium text-neutral-500">
              Exibindo <span className="font-bold text-neutral-black">{paginatedProducts.length}</span> de {filteredProducts.length}
            </span>
